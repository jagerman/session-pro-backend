...
def test_google_process_notification_message(monkeypatch, pg_database):
    ...
    def seed_user_error(conn, token):
        with db.transaction(conn) as tx:
            db.query(tx.conn, 'INSERT INTO user_errors (payment_provider, payment_id, errored_at) VALUES (%s, %s, %s)',
...
def test_stale_revocation_is_not_served(pg_database):
    ...
    with db.connection(pool) as conn:
        ...
        with db.transaction(conn) as tx:
            db.query(tx.conn, "UPDATE generations SET revoked_at = %s WHERE id = %s", now - datetime.timedelta(seconds=1),              gen_recent)
            db.query(tx.conn, "UPDATE generations SET revoked_at = %s WHERE id = %s", now - datetime.timedelta(seconds=RETAIN_FOR + 1), gen_stale)
...
def test_server_add_payment_flow(monkeypatch, pg_database):
    ...
    with contextlib.nullcontext():
        ...
        if 1: # Get the revocation list
            ...
            with db.transaction(db_conn) as tx:
                revoked = backend.add_google_revocation(tx,
        ...
        if 1:
            ...
            backend.update_payment_renewal_info(db_conn,
                                                    payment_tx,
            ...
            with pytest.raises(base.FailError) as exc_info:
                backend.generate_pro_proof(conn          = db_conn,
                                               signing_key   = backend_key,
                                               master_pkey   = master_key.verify_key,
                                               rotating_pkey = rotating_key.verify_key,
                                               request_at    = base.datetime_from_unix_ms(unix_ts_ms),
...
def test_platform_apple(pg_database):
    ...
    with TestingContext(pg_database) as test:
        ...
        err = base.ErrorSink()
        with test.connection() as conn:
            platform_apple.handle_notification(decoded_notification=notification, conn=conn, notification_retry_duration=datetime.timedelta(0), err=err)
            assert not err.has(), err.msg_list
        ...
        response = test.flask_client.post(server.FLASK_ROUTE_ADD_PRO_PAYMENT, json={
            'master_pkey':   bytes(master_key.verify_key).hex(),
            'rotating_pkey': bytes(rotating_key.verify_key).hex(),
            'master_sig':    bytes(master_key.sign(payment_hash_to_sign).signature).hex(),
            'rotating_sig':  bytes(rotating_key.sign(payment_hash_to_sign).signature).hex(),
        ...
        proof_hash = backend.build_proof_message(result_revocation_tag, result_rotating_pkey, base.datetime_from_unix_seconds(result_expiry_ts))
    ...
    with TestingContext(pg_database) as test:
        if 1: # Subscribe notification
            ...
            err = base.ErrorSink()
            with test.connection() as conn:
                platform_apple.handle_notification(decoded_notification=decoded_notification, conn=conn, notification_retry_duration=datetime.timedelta(0), err=err)
                assert not err.has(), err.msg_list
        ...
        if 1: # Expire (voluntary) notification
            ...
            with test.connection() as conn:
                backend.expire_payments_revocations_and_users(conn=conn, now=payment_list[0].expires_at + datetime.timedelta(milliseconds=1))
    ...
    with TestingContext(pg_database) as test:
        ...
        if 1:
            ...
            with test.connection() as conn:
                platform_apple.handle_notification(decoded_notification=e00_sub_to_3_months_decoded_notification, conn=conn, notification_retry_duration=datetime.timedelta(0), err=err)
                assert not err.has(), err.msg_list
        ...
        with test.connection() as conn:
            platform_apple.handle_notification(decoded_notification=e01_upgrade_to_1wk_decoded_notification, conn=conn, notification_retry_duration=datetime.timedelta(0), err=err)
            assert not err.has(), err.msg_list
            payment_list = backend.get_payments_list(conn)
        ...
        with test.connection() as conn:
            platform_apple.handle_notification(decoded_notification=e02_disable_auto_renew_decoded_notification, conn=conn, notification_retry_duration=datetime.timedelta(0), err=err)
            assert not err.has(), err.msg_list
        ...
        with test.connection() as conn:
            platform_apple.handle_notification(decoded_notification=e03_queue_downgrade_to_3_months_decoded_notification,  conn=conn, notification_retry_duration=datetime.timedelta(0), err=err)
            assert not err.has(), err.msg_list
        ...
        with test.connection() as conn:
            platform_apple.handle_notification(decoded_notification=e04_cancel_downgrade_to_3_months_decoded_notification, conn=conn, notification_retry_duration=datetime.timedelta(0), err=err)
            assert not err.has(), err.msg_list
        ...
        with test.connection() as conn:
            platform_apple.handle_notification(decoded_notification=e05_disable_auto_renew_decoded_notification, conn=conn, notification_retry_duration=datetime.timedelta(0), err=err)
            assert not err.has(), err.msg_list
        ...
        with test.connection() as conn:
            platform_apple.handle_notification(decoded_notification=e06_expire_voluntary_decoded_notification,   conn=conn, notification_retry_duration=datetime.timedelta(0), err=err)
            assert not err.has(), err.msg_list
    ...
    with TestingContext(pg_database) as test:
        ...
        with test.connection() as conn:
            platform_apple.handle_notification(decoded_notification=e01_consumption_req_decoded_notification, conn=conn, notification_retry_duration=datetime.timedelta(0), err=err)
            assert not err.has(), err.msg_list
        ...
        with test.connection() as conn:
            platform_apple.handle_notification(decoded_notification=e02_apple_refund_decoded_notification, conn=conn, notification_retry_duration=datetime.timedelta(0), err=err)
            assert not err.has(), err.msg_list
...
def test_google_platform_handle_notification(monkeypatch, pg_database):
    with TestingContext(pg_database) as ctx:
        platform_google.init(cloud_project_id                  = 'loki-5a81e',
...
current_state={'kind': 'androidpublisher#subscriptionPurchaseV2', 'startTime': '2025-10-10T02:51:47.820Z', 'regionCode': 'AU', 'subscriptionState': 'SUBSCRIPTION_STATE_ACTIVE', 'latestOrderId': 'GPA.3346-9218-7706-30541..0', 'linkedPurchaseToken': 'djmaggipjlbmnncfpnaiecgp.AO-J1OzsodQ6LAqNSpZq4F8pvQCko4BhEvKfI8x4JU95p3v0lVVEIis2J-L8WwifcHwYGuCl0fZ4Tjby9Cyig9R5NUYVGqq156Gezco_-Dbw-pyHAZWVM3E', 'testPurchase': {}, 'acknowledgementState': 'ACKNOWLEDGEMENT_STATE_ACKNOWLEDGED',
  ...
  'lineItems': [{'productId': 'session_pro', 'expiryTime': '2025-10-10T03:06:07.125Z', 'autoRenewingPlan': {'autoRenewEnabled': True, 'recurringPrice': {'currencyCode': 'AUD', 'units': '84', 'nanos': 990000000}}, 'offerDetails': {'basePlanId': 'session-pro-3-months', 'offerTags': ['three-months']}, 'latestSuccessfulOrderId': 'GPA.3346-9218-7706-30541..0'}]}
        ...
        """2. User changes to 3-month plan"""
        test_make_purchase_and_claim_payment(purchase=change_plan_a, plan=base.ProPlan.ThreeMonth, user_ctx=user_ctx, ctx=ctx)
