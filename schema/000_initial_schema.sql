...
-- revocation list (the wire `revocation_tag`); it replaces the old gen_index counter + BLAKE2b hash.
-- A revocation IS `revoked_at` being set (no separate revocations table); the served revocation list is
-- the rows with revoked_at set, within the retention window.
