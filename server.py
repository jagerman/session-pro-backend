            ...
            with db.transaction(conn) as tx:
                ...
                if ticket < revocation_ticket:
                    ...
                    for row in db.query(tx.conn, "SELECT gen_index, created_at, expires_at FROM revocations WHERE expires_at > %s", now):
                        ...
                        # The per-entry entitlement expiry is deliberately NOT emitted — clients age
                        # entries out via the list-level retain_for below, not a per-entry expiry.
                        revocation_items.append({
                            'revocation_tag': gen_index_hash.hex(),
                            # Integer seconds: a day-aligned/computed instant (wire spec §1/§4).
